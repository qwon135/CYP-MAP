...
class SOMPredictor(torch.nn.Module):
    def __init__(self, channels, dropout, n_classes, cyp_list):
        ...
        for cyp in self.cyp_list:
            self.reaction_head[cyp] = Attention(channels, dropout, n_classes)  # 반응 여부 예측 (0: 반응 없음, 1: 반응 있음)

    def forward(self, x):
        x = self.proj_edge(x)
        logits = {}
        for cyp in self.cyp_list:            
            logits[cyp] = self.reaction_head[cyp](x)
        return logits   
    
class TypePredictor(torch.nn.Module):
    def __init__(self, channels, dropout, n_classes, cyp_list):
        super().__init__()
        self.cyp_list = cyp_list
        self.proj_edge = torch.nn.Sequential(
...
class GNNSOM(torch.nn.Module):
    def __init__(self, 
        ...
        self.atom_tasks = ['spn', 'H',  'nh_oxi']
        self.bond_tasks = ['clv','nn_oxi', 'rdc']

    
        self.atom_som_fc = SOMPredictor(latent_size, dropout_som_fc, 1, cyp_list) # Any Reaction, spn-oxidation, hydroxylation, n-h Oxidation
        self.bond_som_fc = SOMPredictor(latent_size, dropout_som_fc, 1, cyp_list) # Any reaction, Cleavage, n-n Oxidation, Reduction        

        self.atom_type_fc = TypePredictor(latent_size, dropout_type_fc, 3, cyp_list) # Any Reaction, spn-oxidation, hydroxylation, n-h Oxidation
        self.bond_type_fc = TypePredictor(latent_size, dropout_type_fc, 3, cyp_list) # Any reaction, Cleavage, n-n Oxidation, Reduction
    ...
    def forward(self, batch):
        ...
        bond_som_logits = self.bond_som_fc(edge_attr)
        atom_som_logits = self.atom_som_fc(x)        

        bond_type_logits = self.bond_type_fc(edge_attr)
        atom_type_logits = self.atom_type_fc(x)        

        for cyp_idx, cyp in enumerate(self.cyp_list):
            logits['subs'][cyp] = substrate_logit[:, cyp_idx]
            
            logits['atom'][cyp] = atom_som_logits[cyp].squeeze(-1)
            logits['bond'][cyp] = bond_som_logits[cyp].squeeze(-1)
                    
            logits['clv'][cyp] = bond_type_logits[cyp][:, 0]
            logits['nn_oxi'][cyp] = bond_type_logits[cyp][:, 1]
            logits['rdc'][cyp] = bond_type_logits[cyp][:, 2]
        
            logits['spn'][cyp] = atom_type_logits[cyp][:, 0]
            logits['H'][cyp] = atom_type_logits[cyp][:, 1]
            logits['nh_oxi'][cyp] = atom_type_logits[cyp][:, 2]
    ...
    def forward_with_loss(self, batch, loss_fn_ce, loss_fn_bce, device, args):
        ...
        for cyp_idx, cyp in enumerate(self.cyp_list):
            ...
            if args.reduction == 'sum':
                ...
                loss_dict[f'{cyp}_clv_loss'] = loss_fn_bce(logits['clv'][cyp][not_has_H_bond], labels['clv'][not_has_H_bond]) / labels['clv'].shape[0] if not_has_H_bond.sum() else 0
                loss_dict[f'{cyp}_rdc_loss'] = loss_fn_bce(logits['rdc'][cyp][not_has_H_bond], labels['rdc'][not_has_H_bond]) / labels['rdc'].shape[0] if not_has_H_bond.sum() else 0
            else:                
