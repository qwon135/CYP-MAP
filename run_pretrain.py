...
class MoleculeDataset_graphcl(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        data = torch.load(f'pretrain_data/graph_pt/{sid}.pt')
...
def main(args):
    ...
    data = pd.read_parquet('pretrain_data/data.parquet')
    ...
    train_loader = DataLoader(train_dataset, sampler=DistributedSampler(train_dataset),  batch_size=args.batch_size // args.n_devices, shuffle=False, num_workers=args.n_threads,)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for step, (_, batch1, batch2) in enumerate(train_loader):
            ...
            if (step % 50) == 0 and (local_rank == 0) and (step != 0):
                step_loss /= 50
        ...
        if local_rank == 0:
            if train_loss < best_val_loss:
                ...
                torch.save(model_without_ddp.gnn.state_dict(), f'ckpt_pretrain/gnn_pretrain.pt')

            torch.save(
                {
                'optimizer_state_dict': optim.state_dict(),
                'model_state_dict': model_without_ddp.state_dict(),
                'gnn_state_dict' : model_without_ddp.gnn.state_dict(),
